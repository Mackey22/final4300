...
def gen_data_file(minReviews=10, cutoff=5000, n_feats=5000, topNToFind=10):
    start = time.time()
    ...
    restaurant_by_vocab_matrix = io.mmread("topicmodel/doc_topic_small.mtx").tocsr()
    with open('topics_dt_mtx/unique_ids.json') as fp:
        unique_ids = json.load(fp)
    ordered_business_categories = []
...
if __name__ == "__main__":
    ...
    gen_data_file(minReviews=10, cutoff=500, n_feats=5000, topNToFind=5)# Uncomment this to run preprocessing: Generates data file with sim matrix, business id/name dicts, and unique_ids for indexing business ids in sim matrix
